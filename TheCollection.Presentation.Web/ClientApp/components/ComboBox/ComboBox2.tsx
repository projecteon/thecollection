...
import { ItemsList } from './ItemsList2';
...
export class ComboBox<T> extends Component<IComboBoxProps<T>, IComboBoxState<T>> {
  ...
  componentWillReceiveProps(props: IComboBoxProps<T>) {
    if (props.selectedItem === undefined || props.selectedItem === null) {
      this.setState(
        produce<IComboBoxState<T>>(draft => {
          draft.displayResults = false;
          draft.searchTerm = '';
          draft.searchedTerm = '';
      ...
      this.setState(
        produce<IComboBoxState<T>>(draft => {
          draft.displayResults = false;
          draft.searchTerm = props!.selectedItem![props.displayProperty];
          draft.searchedTerm = props!.selectedItem![props.displayProperty];
          draft.selectedItem = props.selectedItem;
          draft.results = [props.selectedItem!];
        }),
      );
    }
  }
  ...
  renderResult() {
    ...
    return <ItemsList<T> items={items} renderItem={this.props.renderItem} onItemSelected={this.onItemSelected} />;
  }
